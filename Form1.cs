...
namespace ToodedAB
{
    public partial class Form1 : Form
    {
        ...
        {
            if(cb1.Text.Trim()!=string.Empty && cb2.Text.Trim()!=string.Empty && cb3.Text.Trim() != string.Empty && cb4.Text.Trim() != string.Empty && cb4.SelectedItem!=null)
            {
                try
                {
                    connect.Open();
                    command= new SqlCommand("INSERT INTO Toodetable (Toodenimetus,Kogus,Hind,Pilt,Kategooriad) VALUES (@toode,@kogus,@hind,@pilt,@kat)");
                    command.Parameters.AddWithValue("@toode", cb1.Text);
                    command.Parameters.AddWithValue("@kogus", cb2.Text);
                    command.Parameters.AddWithValue("@hind", cb3.Text);
                    command.Parameters.AddWithValue("@pilt", cb1.Text+".jpg");
                    command.Parameters.AddWithValue("@kat", cb4.SelectedIndex+1);
